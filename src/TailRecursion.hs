...
-- | Determine if the given CpsExp only refers to the given raw binders
...
onlyTailCalls :: CpsExp -> [Name] -> Bool
onlyTailCalls exp names = go exp
  where
    ...
    go (LamCps _ _ body k) = not (body `refersTo` names) && goCont k
      where goCont (VarCont _) = True
            goCont (FnCont _ ke) = go ke

    simpleExpTargets = map mkRawVar names
    safe = (`notElem` simpleExpTargets)

-- | Determine if the given CpsExp refers to the given raw binders
-- _at all_, in any position anywhere in the structure.
--
...
refersTo :: CpsExp -> [Name] -> Bool
refersTo e ns = anyOf biplate (`elem` map mkRawVar ns) e